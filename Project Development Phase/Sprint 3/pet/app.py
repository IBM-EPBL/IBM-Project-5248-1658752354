...
@app.route("/monthlyspending",methods=["POST","GET"])
def monthlyspending():
    if request.method == "GET":
        ...
        #  display expenses 
        conn = ibm_db.connect(get_db_credential()," "," ") 
        sql = "SELECT * FROM expense WHERE uname= ? and ( VARCHAR_FORMAT (dateofexpense,'YYYY-MM-DD') > ? and VARCHAR_FORMAT (dateofexpense,'YYYY-MM-DD')< ?);"
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,dateofexpense)
        ibm_db.bind_param(stmt,3,enddate)
        ...
        while ibm_db.fetch_row(stmt)!= False:     
            for i in range(6):
                 lis3.append(ibm_db.result(stmt,i))
            expenselist2.append(lis3) 
            lis3=[]     
            
        currentmonth=fullmonth()   
    
        return render_template("monthly-spending.html",remaining=remaining,expense=sum,expenselist2=expenselist2,currentmonth=currentmonth)
    


@app.route("/spendingcategory",methods=["POST","GET"])
def spendingcategory():
    if request.method == "GET":
        ...
        conn = ibm_db.connect(get_db_credential()," "," ") 
        sql = "SELECT amount FROM expense WHERE ( uname= ? and category= ? ) and ( VARCHAR_FORMAT (dateofexpense,'YYYY-MM-DD') > ? and VARCHAR_FORMAT (dateofexpense,'YYYY-MM-DD')< ?);"
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'groceries')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'housing')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'utilities')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'diningout')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'travel')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'entertainment')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'others')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,'savings')
        ibm_db.bind_param(stmt,3,dateofexpense)
        ibm_db.bind_param(stmt,4,enddate)
        ibm_db.execute(stmt)
        ...
        total= int(groc)+int(hous)+int(util)+int(din)+int(shop)+int(trav)+int(ent)+int()+int(other)+int(sav)
        currentmonth=fullmonth()   
    
        # return render_template("monthly-spending.html",remaining=remaining,expense=sum,expenselist2=expenselist2,currentmonth=currentmonth)
        if not total:
            error="Currently no expenses"
            return render_template("spending-categories.html",error=error)
        return render_template("spending-categories.html",currentmonth=currentmonth,total=total,groc=groc,hous=hous,util=util,din=din,shop=shop,trav=trav,ent=ent,other=other,sav=sav)

@app.route("/budgetoverview",methods=["POST","GET"])
def budgetoverview():
    if request.method == "GET":
        ...
        return render_template("budget-overview.html",amount=amount,remaining=remaining,expense=sum,expenselist2=expenselist2,currentmonth=currentmonth)
    
   
@app.route("/expensereport",methods=["POST","GET"])
def expensereport():
    if request.method == "POST":
        month = request.form['month']
        year = request.form['year']
        error="Currently no expenses"
        startdate=f'{year}-{month}-00'
        edate=f'{year}-{month}-31'
        expenselist=[]
        lis1=[]
         #  display expenses 
        conn = ibm_db.connect(get_db_credential()," "," ") 
        ...
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt,1,nameOfUser)
        ibm_db.bind_param(stmt,2,startdate)
        ibm_db.bind_param(stmt,3,edate)
        ibm_db.execute(stmt)
        ...
        while ibm_db.fetch_row(stmt)!= False:     
            ...
            error=False 
        if error:
            return render_template("expense-report.html",error=error)   
      
        return render_template("expense-report.html",expenselist=expenselist)
